...
def set_settings():
        ...
        if not data:
                ...
                j = json.loads(data)
                try:
                    j['settings']['zarafa']['v1']['main']['keycontrols_enabled'] = False
                except KeyError:
                    print "User has not logged into webapp, unable to disable keyboard shortcuts"
                    return
                ...
                if write_settings:
                    print "Disabling keyboard shortcuts for user '%s'" % sys.argv[1]
                else:
