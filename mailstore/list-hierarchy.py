...
class printer:
    ...
    def processFolder(self, folder, name, indent = '  '):
        ...
        while True:
            ...
            if len(rows) == 0:
                break
            for row in rows:
                self.processFolder(self.store.OpenEntry(row[0].Value, IID_IMAPIFolder, MAPI_MODIFY|MAPI_DEFERRED_ERRORS), row[1].Value, indent + '  ')
    ...
    def processStore(self):
        ...
        self.store = self.session.OpenMsgStore(0, storeid, IID_IMsgStore, MDB_WRITE|MAPI_DEFERRED_ERRORS)
        ipmsubtreeid = self.store.GetProps([PR_IPM_SUBTREE_ENTRYID], 0)[0]
        if PROP_TYPE(ipmsubtreeid.ulPropTag) == PT_ERROR:
            print "Unable to get the IPM subtree entryid. hr=%08x" % ipmsubtreeid.Value
            return
        ipmsubtree = self.store.OpenEntry(ipmsubtreeid.Value, IID_IMAPIFolder, MAPI_MODIFY|MAPI_DEFERRED_ERRORS)
        ...
        while True:
            ...
            if len(rows) == 0:
                break
            for row in rows:
                self.processFolder(self.store.OpenEntry(row[0].Value, IID_IMAPIFolder, MAPI_MODIFY|MAPI_DEFERRED_ERRORS), row[1].Value)
        print ''
